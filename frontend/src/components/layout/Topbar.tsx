...
export function Topbar() {
  ...
  const apps = [
    ...
    ...(user?.is_admin ? [{ name: 'Admin', icon: '\uD83D\uDEE1\uFE0F', path: '/admin' }] : []),
  ];
  ...
  return (
    <div className="h-12 bg-[#0078d4] flex items-center px-3 gap-2 shrink-0 relative z-[100]">
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <span className="text-white font-semibold text-[14px]">Maquita Mail</span>
      </div>

      {/* Search bar */}
      <div className="flex-1 max-w-[680px] mx-auto">
        <div className="relative">
          <svg className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-[#605e5c]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <input id="search-input" type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
            ...
            className="w-full h-8 pl-9 pr-4 bg-[#deecf9] text-[13px] text-[#323130] rounded placeholder-[#605e5c] border-none outline-none focus:bg-white focus:shadow-sm transition-colors" />
        </div>
      </div>
      ...
      <div className="flex items-center gap-0.5 shrink-0">
        ...
        <button onClick={() => navigate('/settings')} title="Configuracion"
          className="w-8 h-8 rounded flex items-center justify-center text-white/80 hover:bg-white/15 transition-colors">
          <svg className="w-[18px] h-[18px]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
