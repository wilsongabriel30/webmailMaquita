...
| **Contraseñas de aplicación opcionales** | La política `contrasenas_aplicacion_obligatorias` (tabla `auth_politica`) está en `false` desde el 17/09/2026: la contraseña principal vale también en IMAP/SMTP/DAV desde cualquier IP, y las contraseñas de aplicación siguen disponibles como opción revocable por dispositivo. Motivo: con la política activa ningún cliente (celulares, Outlook) podía conectarse hasta crear una contraseña de aplicación y nadie lo hacía. Los accesos externos siguen protegidos por TLS, fail2ban, el filtro por país del cortafuegos y el bloqueo de cuentas del panel. Volver a `true` exige contraseña de aplicación fuera del servidor sin reiniciar nada. | `auth_politica`, `dovecot-config/auth-sql.conf.ext` |
