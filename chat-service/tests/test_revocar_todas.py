...
def test_todas_sin_generacion_revoca_todo():
    ...
    assert sc.sesion_revocada(7, "sid-A", 0) is True
    assert sc.sesion_revocada(7, "sid-B", 99) is True
...
def test_cuerpo_de_la_guia_con_av_cero_tambien_revoca():
    ...
    assert sc.sesion_revocada(8, "sid-A", 5) is True


def test_con_generacion_se_conserva_el_corte_fino():
    sc.registrar_revocacion(9, "*", 4)
    assert sc.sesion_revocada(9, "sid-A", 3) is True     # anterior: fuera
    assert sc.sesion_revocada(9, "sid-A", 4) is False    # igual o posterior: sigue
    assert sc.sesion_revocada(9, "sid-A", 7) is False


def test_una_sesion_concreta_no_afecta_a_las_demas():
    sc.registrar_revocacion(10, "sid-A")
    assert sc.sesion_revocada(10, "sid-A", 1) is True
    assert sc.sesion_revocada(10, "sid-B", 1) is False


def test_quien_no_tiene_revocaciones_sigue_dentro():
    assert sc.sesion_revocada(11, "sid-A", 1) is False
