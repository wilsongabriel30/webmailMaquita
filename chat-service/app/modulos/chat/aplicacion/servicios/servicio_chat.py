...
class ServicioChat:
    ...
    def obtener_conversaciones(
        ...
        for conv in conversaciones:
            ...
            if conv.es_directa():
                ...
                if otro:
                    dto.nombre = f"Usuario {otro.usuario_id}"  # Se reemplaza en el controlador
    ...
    def _es_master_admin(self, usuario_id: int) -> bool:
        """Verifica si un usuario tiene rol master_admin."""
        try:
            from sqlalchemy import text
            from infraestructura.base_datos.base import obtener_gestor
            gestor = obtener_gestor()
            with gestor.session() as session:
                result = session.execute(
                    text("SELECT role FROM usuarios WHERE id = :user_id"),
                    {'user_id': usuario_id}
                )
                row = result.fetchone()
                if row:
                    role = row[0]
                    return role in ('master', 'master_admin')
        except Exception:
            pass
        return False
    ...
    def editar_mensaje(
        ...
        # Verificar permisos: dueno del mensaje O master_admin
        es_master = self._es_master_admin(usuario_id)
        if not mensaje.puede_editar(usuario_id) and not es_master:
    ...
    def eliminar_mensaje(
        ...
        # Verificar si es moderador de la conversacion
        participante = self._repo_participante.buscar_en_conversacion(
            mensaje.conversacion_id, usuario_id
        )
        es_moderador = participante and participante.es_moderador()

        # Verificar si es master_admin (tiene permisos globales)
        es_master = self._es_master_admin(usuario_id)

        if not mensaje.puede_eliminar(usuario_id, es_moderador) and not es_master:
    ...
    def reenviar_mensaje(self, mensaje_id: int, remitente_id: int, conversacion_destino_id: int) -> RespuestaChat:
        """Reenvia un mensaje existente a otra conversacion."""
        # 1. Obtener mensaje original
        mensaje_original = self._repo_mensaje.buscar_por_id(mensaje_id)
        if not mensaje_original:
            return RespuestaChat(exito=False, mensaje="Mensaje no encontrado")

        # 2. Verificar acceso al mensaje original
        participante_origen = self._repo_participante.buscar_en_conversacion(
            mensaje_original.conversacion_id, remitente_id)
        if not participante_origen or not participante_origen.activo:
            return RespuestaChat(exito=False, mensaje="No tienes acceso a este mensaje")

        # 3. Verificar participacion en destino
        participante_destino = self._repo_participante.buscar_en_conversacion(
            conversacion_destino_id, remitente_id)
        if not participante_destino or not participante_destino.activo:
            return RespuestaChat(exito=False, mensaje="No tienes acceso a la conversacion destino")

        # 4. Crear mensaje reenviado
        nuevo_mensaje = Mensaje(
            id=None,
            public_id=uuid4(),
            conversacion_id=conversacion_destino_id,
            remitente_id=remitente_id,
            contenido=mensaje_original.contenido,
            tipo=TipoMensaje.REENVIADO,
            reenviado_de_id=mensaje_id,
            creado_en=datetime.now()
        )
        nuevo_mensaje = self._repo_mensaje.crear(nuevo_mensaje)

        # 5. Copiar archivos si existen (referencias, no archivos fisicos)
        if mensaje_original.archivos:
            for archivo_orig in mensaje_original.archivos:
                archivo_copia = ArchivoMensaje(
                    id=None,
                    mensaje_id=nuevo_mensaje.id,
                    tipo=archivo_orig.tipo,
                    ruta=archivo_orig.ruta,
                    nombre=archivo_orig.nombre,
                    tamanio=archivo_orig.tamanio,
                    tipo_mime=archivo_orig.tipo_mime,
                    ancho=archivo_orig.ancho,
                    alto=archivo_orig.alto,
                    duracion=archivo_orig.duracion,
                    miniatura_ruta=archivo_orig.miniatura_ruta,
                    orden=archivo_orig.orden
                )
                self._repo_archivo.crear(archivo_copia)

        # 6. Actualizar ultimo mensaje
        self._repo_conversacion.actualizar_ultimo_mensaje(
            conversacion_destino_id,
            f"Reenviado: {(mensaje_original.contenido or 'Archivo')[:30]}",
            nuevo_mensaje.creado_en
        )
        self._incrementar_no_leidos(conversacion_destino_id, remitente_id)

        return RespuestaChat(exito=True, mensaje="Mensaje reenviado", datos={'mensaje': nuevo_mensaje.to_dict()})
    ...
    def fijar_mensaje(self, mensaje_id: int, usuario_id: int) -> RespuestaChat:
        ...
        if len(fijados) >= 3:
            return RespuestaChat(exito=False, mensaje="Máximo 3 mensajes fijados")
