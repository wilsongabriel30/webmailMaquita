...
def test_basta_con_que_una_direccion_sea_privada(monkeypatch):
    monkeypatch.setattr(
        sg, "resolver_todas", _resolver({"mixto.example": ["8.8.8.8", "10.0.0.7"]})
    )
    with pytest.raises(sg.DestinoNoPermitido):
...
def test_http_solo_en_desarrollo(monkeypatch):
    monkeypatch.setattr(sg, "resolver_todas", _resolver({"ok.example": ["8.8.8.8"]}))
    ...
    assert sg.destino_validado("http://ok.example/hook").ip == "8.8.8.8"
...
def test_la_conexion_queda_fijada_a_la_ip_validada(monkeypatch):
    ...
    monkeypatch.setattr(sg, "resolver_todas", _resolver({"hook.example": ["1.1.1.1"]}))
    d = sg.destino_validado("https://hook.example:8443/ruta?x=1")
    assert d.url_fijada == "https://1.1.1.1:8443/ruta?x=1"
    assert d.host == "hook.example" and d.esquema == "https"
    monkeypatch.setattr(
        sg, "resolver_todas", _resolver({"hook.example": ["127.0.0.1"]})
    )
    assert d.url_fijada == "https://1.1.1.1:8443/ruta?x=1"  # no vuelve a resolver
