...
async def export_activities(
    ...
    await log_user_activity(
        db, admin, "email_export", ip_address=ip, details={"type": "activity_export"}
    )
...
async def create_case(
    ...
    await log_user_activity(
        ...
        details={"case_title": body.title, "action": "case_created"},
    )
...
async def get_case(
    case_id: int, request: Request, admin: str = Depends(require_compliance_read)
):
    """Detalle de un caso."""
    db = _get_db(request)
    ...
    searches = await db.fetchval(
        "SELECT count(*) FROM ediscovery_searches WHERE case_id = $1", case_id
    )
    ...
    holds = await db.fetchval(
        "SELECT count(*) FROM legal_holds WHERE case_id = $1 AND is_active", case_id
    )
    exports = await db.fetchval(
        "SELECT count(*) FROM ediscovery_exports WHERE case_id = $1", case_id
    )
...
async def update_case(
    ...
    existing = await db.fetchrow(
        "SELECT * FROM compliance_cases WHERE id = $1", case_id
    )
    if not existing:
        raise HTTPException(404, "Caso no encontrado")
...
async def execute_search(
    ...
    case = await db.fetchrow(
        "SELECT * FROM compliance_cases WHERE id = $1", body.case_id
    )
    if not case:
        raise HTTPException(404, "Caso no encontrado")
    if case["status"] not in ("open", "approved", "in_progress"):
        raise HTTPException(
            400, f"Caso en estado '{case['status']}' — no permite búsquedas"
        )
        ...
        for mailbox_user in mailboxes:
            for folder in body.folders_scope or ["INBOX"]:
                try:
                    ...
                    logger.warning(
                        "Error buscando %s/%s: %s", mailbox_user, folder, exc
                    )
...
async def _search_mailbox_imap(
    ...
    def _do_search():
        try:
            ...
            logger.info(
                "doveadm search found %d UIDs for %s/%s",
                len(lines),
                mailbox_user,
                folder,
            )
        ...
        try:
            ...
            if criteria.keywords:
                ...
                if not any(kw.lower() in text_content for kw in criteria.keywords):
                    ...
                    body_text = await _fetch_message_body_text(
                        mailbox_user, uid, folder, loop
                    )
            ...
            await db.execute(
                ...
                sent_date,
                msg_size,
                msg_data.get("has_attachments", False),
                msg_hash,
            )
            count += 1
            logger.info(
                "eDiscovery: indexed %s/%s uid=%s subject=%s",
                mailbox_user,
                folder,
                uid,
                msg_data.get("subject", "?")[:50],
            )
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "-u",
                    user,
                    "hdr.subject hdr.from hdr.to hdr.message-id hdr.date hdr.content-type date.sent size.physical",
                    "mailbox",
                    folder,
                    "uid",
                    uid,
            ...
            if result.returncode != 0:
                logger.warning(
                    "doveadm fetch failed for %s uid %s: rc=%d stderr=%s",
                    user,
                    uid,
                    result.returncode,
                    result.stderr[:200],
...
async def get_search_results(
    ...
    search = await db.fetchrow(
        "SELECT * FROM ediscovery_searches WHERE id = $1", search_id
    )
    if not search:
        raise HTTPException(404, "Búsqueda no encontrada")

    total = await db.fetchval(
        "SELECT count(*) FROM ediscovery_results WHERE search_id = $1", search_id
    )
    rows = await db.fetch(
    ...
    await log_user_activity(
        ...
        details={"results_viewed": len(rows)},
    )
...
async def list_exports(
    ...
    if case_id:
        rows = await db.fetch(
            "SELECT * FROM ediscovery_exports WHERE case_id = $1 ORDER BY exported_at DESC",
            case_id,
        )
    else:
        rows = await db.fetch(
            "SELECT * FROM ediscovery_exports ORDER BY exported_at DESC LIMIT 100"
        )
...
async def list_holds(
    ...
    if active_only:
        rows = await db.fetch(
            "SELECT * FROM legal_holds WHERE is_active = TRUE ORDER BY enabled_at DESC"
        )
    else:
        rows = await db.fetch(
            "SELECT * FROM legal_holds ORDER BY enabled_at DESC LIMIT 100"
        )
...
async def create_hold(
    ...
    mb = await db.fetchrow(
        "SELECT username FROM mailbox WHERE username = $1", body.mailbox
    )
    if not mb:
...
async def release_hold(
    ...
    await log_user_activity(
        ...
        "legal_hold_release",
        ip_address=ip,
        mailbox=hold["mailbox"],
        target=f"case:{hold['case_id']}/hold:{hold_id}",
    )
        ...
        "alerts": [
            {
                k: (
                    v.isoformat()
                    if isinstance(v, datetime)
                    else (str(v) if k == "source_ip" and v else v)
                )
                for k, v in dict(r).items()
            }
            for r in rows
...
async def link_alert_to_case(
    ...
    db = _get_db(request)
    await db.execute(
        "UPDATE fraud_alerts SET case_id = $1 WHERE id = $2", case_id, alert_id
    )
...
async def alert_stats(
    ...
    total = await db.fetchval(
        "SELECT count(*) FROM fraud_alerts WHERE created_at >= NOW() - make_interval(days => $1)",
        days,
    )
...
async def mail_trace_stats(
    ...
    total = await db.fetchval(
        "SELECT count(*) FROM mail_trace WHERE created_at >= NOW() - make_interval(hours => $1)",
        hours,
    )
