...
class PostureAgent(Agent):
    name = "postura"
    descripcion = (
        "Audita la postura de seguridad del correo y recomienda mejoras con IA."
    )
    ...
    async def run(self, ctx, dry_run: bool = True) -> dict:
        ...
        facts = {
            ...
            "safelinks_activo": await val(
                "SELECT enabled FROM safelinks_config WHERE id=1"
            ),
            "con_2fa": await val("SELECT count(*) FROM user_totp WHERE enabled") or 0,
            "politicas_retencion": await val(
                "SELECT count(*) FROM retention_policies WHERE is_active"
            )
            or 0,
            "auto_contencion": await val(
        ...
        return {
            ...
            "summary": "Auditoría de postura de seguridad",
            "facts": facts,
            "actions": [],
            "ai": rec,
        }
