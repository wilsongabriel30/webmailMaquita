...
class InboxTriageAgent(Agent):
    ...
    async def run(self, ctx, dry_run: bool = True) -> dict:
        ...
        if not user or "@" not in user:
            return {
                **base,
                "summary": "Indica el buzón a triar (params user)",
                "items": [],
            }
        ...
        try:
            imap = await get_imap_connection(
                f"{user}*admin", ctx.settings.master_password
            )
        except Exception as e:
            return {
                **base,
                "summary": f"No se pudo abrir el buzón de {user}: {e}",
                "items": [],
            }
        try:
        ...
        if not msgs:
            return {
                **base,
                "summary": f"Bandeja de {user}: sin correos recientes",
                "items": [],
            }
            ...
            items.append(
                {
                    ...
                    "categoria": cat,
                    "prioridad": c.get("prioridad", "media"),
                }
            )
        return {
            **base,
            "summary": f"Bandeja de {user}: {len(items)} correos triados",
            "facts": counts,
            "items": items,
        }
