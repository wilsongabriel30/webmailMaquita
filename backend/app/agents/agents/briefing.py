    ...
    async def run(self, ctx, dry_run: bool = True) -> dict:
        ...
        d = {
            "logins_riesgo_alto": await cnt(
                "SELECT count(*) FROM risky_logins WHERE created_at > now()-interval '24 hours' AND lower(risk)='high'"
            ),
            ...
            "clics_peligrosos": await cnt(
                "SELECT count(*) FROM safelinks_clicks WHERE created_at > now()-interval '24 hours' AND lower(coalesce(verdict,'')) IN ('suspicious','malicious','blocked')"
            ),
            "incidentes_air": await cnt(
                "SELECT count(*) FROM threat_actions WHERE actor='AIR' AND created_at > now()-interval '24 hours'"
            ),
        ...
        return {
            ...
            "summary": "Parte diario de seguridad",
            "facts": d,
            "actions": [],
            "ai": texto,
        }
