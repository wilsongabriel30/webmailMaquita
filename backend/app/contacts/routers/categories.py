...
async def update_category(
    cat_id: int, request: Request, username: str = Depends(get_current_user)
):
...
async def delete_category(
    ...
    await db.execute(
        "DELETE FROM contact_category_assignments WHERE category_id=$1", cat_id
    )
    await db.execute(
        "DELETE FROM contact_categories WHERE id=$1 AND owner=$2", cat_id, username
    )
...
async def assign_categories(
    ...
    await db.execute(
        "DELETE FROM contact_category_assignments WHERE contact_id=$1", contact_id
    )
    for cid in cat_ids:
        await db.execute(
