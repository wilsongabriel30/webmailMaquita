...
async def restore_contact(
    ...
    if dup:
        raise HTTPException(
            409, f"Ya existe un contacto activo con el email {row['email']}"
        )
...
async def permanent_delete(
    ...
    if r == "DELETE 0":
        raise HTTPException(
            404, "Solo se pueden eliminar permanentemente contactos en papelera"
        )
...
async def empty_trash(request: Request, username: str = Depends(get_current_user)):
    ...
    db = request.app.state.db_pool
    r = await db.execute(
        "DELETE FROM user_contacts WHERE owner=$1 AND deleted_at IS NOT NULL", username
    )
