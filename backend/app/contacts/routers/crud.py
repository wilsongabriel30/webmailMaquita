...
async def list_contacts(
    ...
    count = await db.fetchval(
        f"SELECT COUNT(*) FROM user_contacts WHERE {where}", *params
    )
...
async def create_contact(request: Request, username: str = Depends(get_current_user)):
    ...
    """,
        ...
        body.get("email2", ""),
        body.get("email3", ""),
        body.get("phone_mobile", ""),
        body.get("phone_work", ""),
        body.get("phone_home", ""),
        body.get("fax", ""),
        body.get("address_street", ""),
        body.get("address_city", ""),
        body.get("address_state", ""),
        body.get("address_zip", ""),
        body.get("address_country", ""),
        birthday,
        body.get("website", ""),
        body.get("im_address", ""),
        body.get("photo_url", ""),
...
async def update_contact(
    ...
    existing = await db.fetchrow(
        "SELECT * FROM user_contacts WHERE id=$1 AND owner=$2", contact_id, username
    )
    if not existing:
    ...
    """,
        ...
        body.get("organization", existing["organization"]),
        body.get("notes", existing["notes"]),
        body.get("first_name", existing["first_name"]),
        body.get("last_name", existing["last_name"]),
        body.get("nickname", existing["nickname"]),
        body.get("job_title", existing["job_title"]),
        body.get("department", existing["department"]),
        body.get("company", existing["company"]),
        body.get("email2", existing["email2"]),
        body.get("email3", existing["email3"]),
        body.get("phone_mobile", existing["phone_mobile"]),
        body.get("phone_work", existing["phone_work"]),
        body.get("phone_home", existing["phone_home"]),
        body.get("fax", existing["fax"]),
        body.get("address_street", existing["address_street"]),
        body.get("address_city", existing["address_city"]),
        body.get("address_state", existing["address_state"]),
        body.get("address_zip", existing["address_zip"]),
        body.get("address_country", existing["address_country"]),
        birthday,
        body.get("website", existing["website"]),
        body.get("im_address", existing["im_address"]),
