    ...
    for key, group in groups_map.items():
        if len(group) >= 2:
            ...
            if ids not in seen_ids:
                seen_ids.add(ids)
                domain = key.split("|")[0]
                result.append({"reason": f"Nombre similar en @{domain}", "contacts": group})
...
async def merge_contacts(request: Request, username: str = Depends(get_current_user)):
    ...
    fillable = [
        "first_name", "last_name", "nickname", "email", "email2", "email3",
        "phone", "phone_mobile", "phone_work", "phone_home", "fax",
        "organization", "company", "job_title", "department",
        "address_street", "address_city", "address_state", "address_zip", "address_country",
        "birthday", "website", "im_address", "photo_url", "notes"
    ]
    updates = []
    values = [keep_id]
    idx = 2
    for field in fillable:
        ...
        if (not keep_val or (isinstance(keep_val, str) and not keep_val.strip())) and merge_val:
