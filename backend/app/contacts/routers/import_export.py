...
_CSV_MAP = {
    ...
    "telefono trabajo": "phone_work",
    "teléfono trabajo": "phone_work",
    "home phone": "phone_home",
    "telefono casa": "phone_home",
    "teléfono casa": "phone_home",
    "business fax": "fax",
    "company": "company",
    "empresa": "company",
    ...
    "zip": "address_zip",
    "codigo postal": "address_zip",
    "código postal": "address_zip",
...
async def import_contacts(request: Request, username: str = Depends(get_current_user)):
    ...
    if line_count > MAX_IMPORT_CONTACTS + 1:  # +1 for header
        raise HTTPException(
            400,
            f"Máximo {MAX_IMPORT_CONTACTS} contactos por importación (archivo tiene ~{line_count-1})",
        )
...
async def export_contacts(
    ...
    if filter == "favorites":
        where, params = "owner = $1 AND deleted_at IS NULL AND is_favorite = true", [
            username
        ]
    ...
    rows = await db.fetch(
        f"SELECT * FROM user_contacts WHERE {where} ORDER BY display_name", *params
    )
...
async def _merge_existing(
    db, contact_id: int, owner: str, mapped: dict, display_name: str
):
    ...
    for field in _MERGE_FIELDS:
        if mapped.get(field):
            sets.append(
                f"{field} = CASE WHEN COALESCE({field}, '') = '' THEN ${pi} ELSE {field} END"
            )
            params.append(mapped[field])
            pi += 1
    if display_name:
        sets.append(
            f"display_name = CASE WHEN COALESCE(display_name, '') = '' THEN ${pi} ELSE display_name END"
        )
        params.append(display_name)
        pi += 1
    if sets:
        await db.execute(
            f"UPDATE user_contacts SET {', '.join(sets)} WHERE id=$1 AND owner=$2",
            *params,
        )


async def _insert_new(
    db, owner: str, email: str, display_name: str, mapped: dict, birthday
):
    ...
    """,
        ...
        mapped.get("phone", ""),
        mapped.get("phone_mobile", ""),
        mapped.get("phone_work", ""),
        mapped.get("phone_home", ""),
        mapped.get("fax", ""),
        mapped.get("organization", ""),
        mapped.get("company", ""),
        mapped.get("job_title", ""),
        mapped.get("department", ""),
        mapped.get("notes", ""),
        mapped.get("address_street", ""),
        mapped.get("address_city", ""),
        mapped.get("address_state", ""),
        mapped.get("address_zip", ""),
...
def _export_vcard(rows, today: str) -> StreamingResponse:
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": f"attachment; filename=contactos_maquita_{today}.vcf"
        },
    )
...
def _export_csv(rows, today: str) -> StreamingResponse:
        ...
        writer.writerow(
            {
                ...
                "E-mail Address": r.get("email", ""),
                "E-mail 2 Address": r.get("email2", ""),
                "E-mail 3 Address": r.get("email3", ""),
                "Mobile Phone": r.get("phone_mobile", ""),
                "Business Phone": r.get("phone_work", ""),
                ...
                "Web Page": r.get("website", ""),
                "Business Fax": r.get("fax", ""),
                "Street": r.get("address_street", ""),
                "City": r.get("address_city", ""),
                "State": r.get("address_state", ""),
                "Zip": r.get("address_zip", ""),
    ...
    return StreamingResponse(
        ...
        media_type="text/csv",
        headers={
            "Content-Disposition": f"attachment; filename=contactos_maquita_{today}.csv"
        },
    )
