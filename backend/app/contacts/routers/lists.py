...
async def update_list(
    list_id: int, request: Request, username: str = Depends(get_current_user)
):
...
async def delete_list(
    ...
    db = request.app.state.db_pool
    await db.execute(
        "DELETE FROM contact_lists WHERE id=$1 AND owner=$2", list_id, username
    )
...
async def add_members(
    ...
    exists = await db.fetchval(
        "SELECT id FROM contact_lists WHERE id=$1 AND owner=$2", list_id, username
    )
    if not exists:
...
async def remove_member(
    list_id: int, cid: int, request: Request, username: str = Depends(get_current_user)
):
