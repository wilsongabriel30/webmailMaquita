...
async def limpieza_ocasional(db, politica: dict) -> None:
    ...
    try:
        await db.execute(
            "DELETE FROM disp_ubicaciones WHERE recibida_en < NOW() - make_interval(days => $1)",
...
async def enviar_ubicaciones(
    request: Request, body: LoteUbicaciones, equipo: dict = Depends(equipo_actual)
):
...
async def avistamientos(
    request: Request, body: Avistamientos, equipo: dict = Depends(equipo_actual)
):
    ...
    for v in body.vistos:
        perdido = await db.fetchval(
            "SELECT id FROM disp_equipos WHERE baliza_id = $1 AND estado = 'perdido' AND id <> $2",
            v.baliza_id,
            equipo["id"],
        )
        if perdido is None:
        ...
        n += 1
        logger.warning(
            "equipo_perdido_avistado | perdido=%s | por=%s | rssi=%s",
            perdido,
            equipo["id"],
            v.rssi,
        )
