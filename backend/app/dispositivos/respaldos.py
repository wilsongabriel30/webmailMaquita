...
async def _respaldo_abierto(db, equipo: dict, rid: int):
    r = await db.fetchrow(
        "SELECT id, tipo, estado, iniciado_en FROM disp_respaldos WHERE id = $1 AND equipo_id = $2",
        rid,
        equipo["id"],
    )
    if r is None:
...
async def _origenes(db, equipo: dict) -> list[int]:
    ...
    filas = await db.fetch(
        "SELECT origen_id FROM disp_restauraciones WHERE destino_id = $1 AND anulada_en IS NULL AND caduca_en > NOW()",
        equipo["id"],
    )
...
@router.post("")
async def iniciar(
    request: Request, body: Inicio, equipo: dict = Depends(equipo_actual)
):
    ...
    try:
        ...
        raise HTTPException(
            503, "El almacén de respaldos no está disponible; se reintentará más tarde"
        )
    db = request.app.state.db_pool
    await db.execute(
        "UPDATE disp_respaldos SET estado = 'incompleto', cerrado_en = NOW() WHERE equipo_id = $1 AND estado = 'abierto'",
        equipo["id"],
    )
    rid = await db.fetchval(
        "INSERT INTO disp_respaldos (equipo_id, tipo, version_app) VALUES ($1,$2,$3) RETURNING id",
        equipo["id"],
        body.tipo,
        body.version_app,
    )
    usado = await db.fetchval(
        "SELECT COALESCE(SUM(tamano), 0) FROM disp_objetos WHERE equipo_id = $1",
        equipo["id"],
    )
    return {
        "id": rid,
        "trozo_bytes": TROZO_SUGERIDO,
        "trama_bytes": objetos.TRAMA,
        "trozo_maximo_bytes": TROZO_MAXIMO,
        "cuota_bytes": equipo.get("cuota_respaldo_gb", 64) * 1024**3,
...
async def manifiesto(
    rid: int, request: Request, body: Manifiesto, equipo: dict = Depends(equipo_actual)
):
    db = request.app.state.db_pool
    await _respaldo_abierto(db, equipo, rid)
    cuota = equipo.get("cuota_respaldo_gb", 64) * 1024**3
    contenidos = {
        a.sha256: a.tamano for a in body.archivos if a.sha256 != objetos.SHA_VACIO
    }
    async with db.acquire() as con, con.transaction():
        await con.executemany(
            ...
            [
                (
                    rid,
                    a.ruta,
                    a.categoria if a.categoria in CATEGORIAS else "otros",
                    a.sha256,
                    a.tamano,
                    _fecha(a.mtime),
                )
        ...
        if total > cuota:
            raise HTTPException(
                413,
                f"El respaldo supera la cuota del equipo ({equipo.get('cuota_respaldo_gb', 64)} GB). Avise a Tecnología.",
            )
    ...
    return {
        "faltan": [
            {"sha256": f["sha256"], "tamano": f["tamano"], "recibido": f["recibido"]}
            for f in filas
        ]
    }
...
async def subir_trozo(
    ...
    obj = await db.fetchrow(
        "SELECT tamano, recibido, estado FROM disp_objetos WHERE equipo_id = $1 AND sha256 = $2",
        equipo["id"],
        sha,
    )
    if obj is None or obj["tamano"] != total:
        raise HTTPException(
            409,
            "Ese contenido no figura en el manifiesto de este equipo (o cambió de tamaño): envíe antes el manifiesto",
        )
    ...
    if offset != obj["recibido"]:
        return JSONResponse(
            status_code=409,
            content={
                "detail": "Desfase: continúe desde `recibido`",
                "recibido": obj["recibido"],
            },
        )
    ...
    if not datos or fin > total or (len(datos) % objetos.TRAMA and fin != total):
        raise HTTPException(
            400,
            "El trozo debe ser múltiplo de 1 MiB, salvo el último, y no pasar del tamaño declarado",
        )
    ...
    try:
        try:
            await asyncio.to_thread(
                objetos.anexar, objetos.carpeta(equipo), sha, offset, bytes(datos)
            )
        except objetos.Desfase:
            await asyncio.to_thread(objetos.borrar, objetos.carpeta(equipo), sha)
            await db.execute(
                "UPDATE disp_objetos SET recibido = 0 WHERE equipo_id = $1 AND sha256 = $2",
                equipo["id"],
                sha,
            )
            return JSONResponse(
                status_code=409,
                content={
                    "detail": "El contenido guardado no cuadra: empiece de nuevo",
                    "recibido": 0,
                },
            )
        if fin < total:
            await db.execute(
                "UPDATE disp_objetos SET recibido = $3 WHERE equipo_id = $1 AND sha256 = $2",
                equipo["id"],
                sha,
                fin,
            )
        ...
        if not await asyncio.to_thread(
            objetos.verificar, objetos.carpeta(equipo), sha, total
        ):
            await asyncio.to_thread(objetos.borrar, objetos.carpeta(equipo), sha)
            await db.execute(
                "UPDATE disp_objetos SET recibido = 0 WHERE equipo_id = $1 AND sha256 = $2",
            ...
            raise HTTPException(
                422,
                "El contenido recibido no coincide con su SHA-256: vuelva a subirlo",
            )
        await db.execute(
            "UPDATE disp_objetos SET recibido = $3, estado = 'completo', completado_en = NOW() WHERE equipo_id = $1 AND sha256 = $2",
            equipo["id"],
            sha,
            total,
        )
...
async def cerrar(
    rid: int, request: Request, body: Cierre, equipo: dict = Depends(equipo_actual)
):
    db = request.app.state.db_pool
    r = await _respaldo_abierto(db, equipo, rid)
    por_categoria = await db.fetch(
        "SELECT categoria, count(*) AS archivos, COALESCE(SUM(tamano), 0) AS bytes FROM disp_respaldo_archivos WHERE respaldo_id = $1 GROUP BY categoria",
        rid,
    )
    ...
    nuevos = await db.fetchrow(
        "SELECT count(*) AS n, COALESCE(SUM(tamano), 0) AS b FROM disp_objetos WHERE equipo_id = $1 AND completado_en >= $2",
        equipo["id"],
        r["iniciado_en"],
    )
    estado = "completo" if faltantes == 0 else "incompleto"
    resumen = {
        c["categoria"]: {"archivos": c["archivos"], "bytes": int(c["bytes"])}
    ...
    await db.execute(
        ...
        rid,
        estado,
        sum(c["archivos"] for c in por_categoria),
        sum(int(c["bytes"]) for c in por_categoria),
        nuevos["n"],
        int(nuevos["b"]),
        faltantes,
        json.dumps(resumen),
        json.dumps([e[:300] for e in body.errores]),
    )
    if estado == "completo" and r["tipo"] == "cierre":
        await db.execute(
            "UPDATE disp_equipos SET respaldo_cierre_en = NOW() WHERE id = $1",
            equipo["id"],
        )
    pol = await db.fetchval(
        "SELECT valor -> 'respaldo' ->> 'instantaneas' FROM disp_config WHERE clave = 'politica'"
    )
    try:
    ...
    logger.info(
        "respaldo_cerrado | equipo=%s | id=%s | estado=%s | faltantes=%s",
        equipo["id"],
        rid,
        estado,
        faltantes,
...
async def listar(request: Request, equipo: dict = Depends(equipo_actual)):
    ...
    filas = await db.fetch(
        """SELECT r.id, r.equipo_id, e.nombre AS equipo_nombre, e.modelo, r.tipo, r.cerrado_en, r.archivos_total, r.bytes_total, r.resumen
           ...
           WHERE r.equipo_id = ANY($1::int[]) AND r.estado = 'completo' ORDER BY r.cerrado_en DESC LIMIT 60""",
        await _origenes(db, equipo),
    )
    return {
        "respaldos": [
            {
                **dict(f),
                "resumen": (
                    json.loads(f["resumen"])
                    if isinstance(f["resumen"], str)
                    else f["resumen"]
                ),
                "propio": f["equipo_id"] == equipo["id"],
            }
            for f in filas
        ]
    }
...
async def _respaldo_legible(db, equipo: dict, rid: int):
    r = await db.fetchrow(
        "SELECT id, equipo_id FROM disp_respaldos WHERE id = $1 AND estado = 'completo'",
        rid,
    )
    if r is None or r["equipo_id"] not in await _origenes(db, equipo):
        raise HTTPException(
            404, "Respaldo no encontrado o sin autorización de restauración vigente"
        )
    return r


@router.get("/{rid}/manifiesto")
async def leer_manifiesto(
    rid: int,
    request: Request,
    despues_de: str = "",
    limite: int = Query(2000, ge=1, le=2000),
    equipo: dict = Depends(equipo_actual),
):
    db = request.app.state.db_pool
    ...
    if r["equipo_id"] != equipo["id"] and not despues_de:
        await db.execute(
            "INSERT INTO disp_eventos (equipo_id, tipo, detalle) VALUES ($1, 'restauracion', $2::jsonb)",
            r["equipo_id"],
            json.dumps({"restaurado_en_equipo": equipo["id"], "respaldo": rid}),
        )
    filas = await db.fetch(
    ...
    return {
        "equipo_origen": r["equipo_id"],
        "archivos": [dict(f) for f in filas],
...
async def descargar(
    sha: str,
    request: Request,
    equipo_origen: Optional[int] = None,
    desde: int = Query(0, ge=0),
    equipo: dict = Depends(equipo_actual),
):
    if not re.fullmatch(_SHA, sha) or desde % objetos.TRAMA:
        raise HTTPException(
            400, "Parámetros inválidos (`desde` debe ser múltiplo de 1 MiB)"
        )
    db = request.app.state.db_pool
    origen = equipo_origen or equipo["id"]
    if origen not in await _origenes(db, equipo):
        raise HTTPException(
            404, "Sin autorización de restauración vigente para ese equipo"
        )
    fila = await db.fetchrow(
        "SELECT o.tamano, e.id, e.carpeta_respaldo FROM disp_objetos o JOIN disp_equipos e ON e.id = o.equipo_id "
        "WHERE o.equipo_id = $1 AND o.sha256 = $2 AND o.estado = 'completo'",
        origen,
        sha,
    )
    ...
    return StreamingResponse(
        objetos.leer(objetos.carpeta(dict(fila)), sha, desde),
