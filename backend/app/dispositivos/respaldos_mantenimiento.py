...
async def depurar(
    db, equipo_id: int, instantaneas: int = 7, carpeta_eq: str | None = None
) -> None:
    try:
        await db.execute(
        ...
        huerfanos = await db.fetch(
            """SELECT o.sha256 FROM disp_objetos o WHERE o.equipo_id = $1 AND o.creado_en < NOW() - interval '1 day' AND NOT EXISTS (
                   ...
                   WHERE r.equipo_id = $1 AND a.sha256 = o.sha256) LIMIT 5000""",
            equipo_id,
        )
        ...
        if huerfanos:
            await db.execute(
                "DELETE FROM disp_objetos WHERE equipo_id = $1 AND sha256 = ANY($2::bpchar[])",
                equipo_id,
                [h["sha256"] for h in huerfanos],
            )
            logger.info(
                "respaldos_depurados | equipo=%s | contenidos=%d",
                equipo_id,
                len(huerfanos),
            )
    except Exception:
