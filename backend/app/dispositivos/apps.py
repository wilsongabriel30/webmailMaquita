...
def _texto_aviso(nombre: str, nota: str | None) -> str:
    ...
    if nota:
        base += f" Motivo: {nota}."
    return base + " Por favor, tócala para desinstalarla. Si crees que es un error, escribe a Tecnología."


async def _reglas(db) -> dict:
    filas = await db.fetch("SELECT tipo, lower(valor) AS valor, accion, nota FROM disp_apps_reglas")
...
def _evaluar(app: App, reglas: dict) -> tuple[str, str, str | None]:
    ...
    for clave, (accion, nota) in ((app.paquete.lower(), reglas["paquete"].get(app.paquete.lower())) if app.paquete.lower() in reglas["paquete"] else (None, (None, None)),):
        if accion == "bloquear":
    ...
    if inst and inst not in conocidos and inst not in ("com.google.android.packageinstaller", "com.android.packageinstaller"):
...
@router.post("/apps")
async def inventario(request: Request, body: Inventario, equipo: dict = Depends(equipo_actual)):
    ...
    async with db.acquire() as con, con.transaction():
        ...
        previas = {r["paquete"]: r["veredicto"] for r in
                   await con.fetch("SELECT paquete, veredicto FROM disp_apps WHERE equipo_id = $1", equipo["id"])}
        for app in body.apps:
            ...
            await con.execute(
                ...
                equipo["id"], app.paquete, app.nombre, app.version, app.instalador, app.firma_sha256,
                json.dumps(app.permisos[:80]), app.sistema, veredicto, motivo)
            if veredicto != "ok":
        ...
        if nuevas_riesgo:
            await con.execute(
                ...
                equipo["id"], json.dumps({"nuevas": nuevas_riesgo, "avisos": [a["paquete"] for a in avisos]}))
            logger.warning("apps_de_riesgo | equipo=%s | nuevas=%d", equipo["id"], nuevas_riesgo)
