...
async def gather(db, days: int = 7) -> dict:
    ...
    return {
        ...
        "config": {
            ...
            "safelinks_activo": bool(
                await cnt("SELECT enabled FROM safelinks_config WHERE id=1")
            ),
            "auto_contencion": bool(
                await cnt(
                    "SELECT auto_disable_on_compromise FROM threat_config WHERE id=1"
                )
            ),
            "politicas_retencion": await cnt(
                "SELECT count(*) FROM retention_policies WHERE is_active"
            ),
        },
        "logins_riesgo_alto": await cnt(
            "SELECT count(*) FROM risky_logins WHERE created_at>now()-($1)::interval AND lower(risk)='high'",
            iv,
        ),
        "top_logins_riesgo": await rows(
            "SELECT username, coalesce(country,'?') pais, count(*) n FROM risky_logins "
            "WHERE created_at>now()-($1)::interval AND lower(risk)='high' GROUP BY 1,2 ORDER BY n DESC LIMIT 5",
            iv,
        ),
        "violaciones_dlp": await cnt(
            "SELECT count(*) FROM dlp_violations WHERE created_at>now()-($1)::interval",
            iv,
        ),
        "top_dlp": await rows(
        ...
        "incidentes_air": await rows(
            ...
            "WHERE actor='AIR' AND created_at>now()-($1)::interval ORDER BY created_at DESC LIMIT 8",
            iv,
        ),
    }
