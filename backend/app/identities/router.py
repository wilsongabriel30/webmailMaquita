...
async def _ensure_default(db, username: str, exclude_id: Optional[int] = None):
    ...
    if exclude_id:
        ...
        has = await db.fetchval(
            "SELECT COUNT(*) FROM user_identities WHERE username=$1 AND is_default=true",
            username,
        )
    if has == 0:
        if exclude_id:
            ...
            oldest = await db.fetchval(
                "SELECT id FROM user_identities WHERE username=$1 ORDER BY created_at LIMIT 1",
                username,
            )
        if oldest:
            await db.execute(
                "UPDATE user_identities SET is_default=true WHERE id=$1", oldest
            )
...
async def list_identities(request: Request, username: str = Depends(get_current_user)):
    ...
    if not rows:
        ...
        row = await db.fetchrow(
            ...
            username,
            username.split("@")[0].replace(".", " ").title(),
            username,
        )
        rows = [row]
...
async def create_identity(
    request: Request, body: IdentityCreate, username: str = Depends(get_current_user)
):
    db = request.app.state.db_pool
    ...
    count = await db.fetchval(
        "SELECT COUNT(*) FROM user_identities WHERE username=$1", username
    )
    if count >= 10:
...
async def update_identity(
    ...
    existing = await db.fetchrow(
        "SELECT * FROM user_identities WHERE id=$1 AND username=$2",
        identity_id,
        username,
    )
    if not existing:
    ...
    if updates:
        ...
        query = (
            f"UPDATE user_identities SET {', '.join(set_parts)} WHERE id=$1 RETURNING *"
        )
...
async def delete_identity(
    identity_id: int, request: Request, username: str = Depends(get_current_user)
):
    db = request.app.state.db_pool
    existing = await db.fetchrow(
        "SELECT * FROM user_identities WHERE id=$1 AND username=$2",
    ...
    if count <= 1:
        raise HTTPException(
            status_code=400, detail="No puedes eliminar la unica identidad"
        )
