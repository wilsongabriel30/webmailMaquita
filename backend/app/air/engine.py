...
async def _auto_enabled(db) -> bool:
    try:
        row = await db.fetchrow("SELECT auto_disable_on_compromise FROM threat_config WHERE id=1")
        return bool(row and row["auto_disable_on_compromise"])
    except Exception:
        return False


async def run_cycle(db, redis, hours: int = 24, use_ai: bool = True,
                    auto_respond: bool = False) -> list[dict]:
    ...
    for username, sig in sorted(users.items(), key=lambda kv: -kv[1]["score"]):
        ...
        ai = await triage_mod.assess(username, sig, pb) if (use_ai and pb["severity"] != "low") else {}
        ...
        if auto_ok and pb["action"] == "lock":
            await responder.lock_account(db, redis, username,
