...
def evaluate(sig: dict) -> dict:
    ...
    if sig.get("risky_high"):
        reasons.append(f"{sig['risky_high']} login(s) de riesgo alto (viaje imposible/geo)")
    if sig.get("dlp"):
    ...
    if sig.get("risky_high") and sig.get("dlp"):
        return {"severity": HIGH, "action": "lock",
                "reasons": reasons,
                "rationale": "Login de riesgo alto + actividad de salida (DLP): patrón de cuenta comprometida."}
    if score >= 8:
        return {"severity": HIGH, "action": "review", "reasons": reasons,
                "rationale": "Acumulación alta de señales de riesgo."}
    if score >= 4:
        return {"severity": MEDIUM, "action": "review", "reasons": reasons,
                "rationale": "Señales de riesgo moderadas."}
    return {"severity": LOW, "action": "monitor", "reasons": reasons,
