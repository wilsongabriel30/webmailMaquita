...
def _row_to_label(row):
    return LabelOut(
        id=row["id"], board_id=row["board_id"], name=row["name"], color=row["color"]
    )
...
class TaskService:
    ...
    async def _user_has_access(self, db, board_id, user):
        ...
        if val:
            return True
        val = await db.fetchval(
            "SELECT 1 FROM task_board_members WHERE board_id = $1 AND user_email = $2",
            board_id,
            user,
        )
    ...
    async def _list_belongs_to_user(self, db, list_id, user):
        row = await db.fetchrow(
            "SELECT board_id FROM task_lists WHERE id = $1", list_id
        )
        if not row:
            return False
    ...
    async def ensure_default_board(self, db, user):
        ...
        row = await db.fetchrow(
            'SELECT id FROM task_boards WHERE "user" = $1 ORDER BY created_at LIMIT 1',
            user,
        )
        if row:
            board_id = row["id"]
        ...
        default_list = await db.fetchrow(
            "SELECT id FROM task_lists WHERE board_id = $1 AND list_type = 'default'",
            board_id,
        )
        if not default_list:
            await db.execute(
    ...
    async def _get_default_list_id(self, db, user):
        ...
        list_id = await db.fetchval(
            "SELECT id FROM task_lists WHERE board_id = $1 AND list_type = 'default' LIMIT 1",
            board_id,
        )
        return list_id
    ...
    async def create_user_list(self, db, user, data: TaskListCreate):
        ...
        max_pos = await db.fetchval(
            "SELECT COALESCE(MAX(position), -1) FROM task_lists WHERE board_id = $1",
            board_id,
        )
        row = await db.fetchrow(
        ...
        await self._log_activity(
            db, board_id, user, "list_created", f"Lista '{data.name}' creada"
        )
    ...
    async def rename_user_list(self, db, user, list_id, data: TaskListUpdate):
        ...
        row = await db.fetchrow(
            "SELECT list_type FROM task_lists WHERE id = $1", list_id
        )
        ...
        cnt = await db.fetchval(
            "SELECT COUNT(*) FROM task_cards WHERE list_id = $1 AND completed = FALSE",
            list_id,
        )
    ...
    async def delete_user_list(self, db, user, list_id):
        ...
        if not await self._list_belongs_to_user(db, list_id, user):
            raise ValueError("List not found")
        row = await db.fetchrow(
            "SELECT list_type, name FROM task_lists WHERE id = $1", list_id
        )
        if row and row["list_type"] == "default":
        ...
        if board_id:
            await self._log_activity(
                db, board_id, user, "list_deleted", f"Lista '{row['name']}' eliminada"
            )
        ...
        if view == "my_day":
            rows = await db.fetch(
                """SELECT tc.* FROM task_cards tc
                   ...
                   ORDER BY tc.completed, tc.position""",
                board_id,
            )
        elif view == "important":
            rows = await db.fetch(
            ...
            rows = await db.fetch(
                """SELECT tc.* FROM task_cards tc
                   ...
                   ORDER BY tc.completed, tc.due_date, tc.position""",
                board_id,
            )
        elif view == "assigned":
            rows = await db.fetch(
    ...
    async def create_task(self, db, user, list_id, data: CardCreate):
        ...
        if not await self._list_belongs_to_user(db, list_id, user):
            raise ValueError("List not found")
        max_pos = await db.fetchval(
            "SELECT COALESCE(MAX(position), -1) FROM task_cards WHERE list_id = $1",
            list_id,
        )
        row = await db.fetchrow(
            ...
            sanitize_html(data.description or ""),
            data.due_date,
            data.priority,
    ...
    async def create_board(self, db, user, data):
        max_pos = await db.fetchval(
            'SELECT COALESCE(MAX(position), -1) FROM task_boards WHERE "user" = $1',
            user,
        )
        row = await db.fetchrow(
    ...
    async def list_members(self, db, user, board_id):
        if not await self._user_has_access(db, board_id, user):
            raise ValueError("Board not found")
        rows = await db.fetch(
            "SELECT * FROM task_board_members WHERE board_id = $1 ORDER BY joined_at",
            board_id,
        )
    ...
    async def add_member(self, db, user, board_id, data):
        ...
        caller_role = await db.fetchval(
            "SELECT role FROM task_board_members WHERE board_id = $1 AND user_email = $2",
            board_id,
            user,
        )
        ...
        await self._log_activity(
            ...
            user,
            "member_added",
            f"{data.user_email} agregado como {data.role}",
        )
    ...
    async def remove_member(self, db, user, board_id, member_email):
        ...
        is_owner = await db.fetchval(
            'SELECT 1 FROM task_boards WHERE id = $1 AND "user" = $2',
            board_id,
            member_email,
        )
        if is_owner:
        ...
        await db.execute(
            "DELETE FROM task_board_members WHERE board_id = $1 AND user_email = $2",
            board_id,
            member_email,
        )
        await self._log_activity(
            db, board_id, user, "member_removed", f"{member_email} removido"
        )
    ...
    async def list_cards(self, db, user, list_id):
        if not await self._list_belongs_to_user(db, list_id, user):
            raise ValueError("List not found")
        rows = await db.fetch(
            "SELECT * FROM task_cards WHERE list_id = $1 ORDER BY position", list_id
        )
        return [_row_to_card(r) for r in rows]
    ...
    async def create_card(self, db, user, list_id, data):
        ...
        if board_id:
            ...
            await self._log_activity(
                db, board_id, user, "card_created", details, row["id"]
            )
        return _row_to_card(row)
    ...
    async def update_card(self, db, user, card_id, data):
        ...
        if board_id:
            ...
            if data.assigned_to is not None and data.assigned_to != old_row.get(
                "assigned_to"
            ):
            ...
            if data.important is not None and data.important != old_row.get(
                "important"
            ):
            ...
            if data.my_day is not None and data.my_day != old_row.get("my_day"):
                changes.append(
                    "agregada a Mi Dia" if data.my_day else "removida de Mi Dia"
                )
            if changes:
                await self._log_activity(
                    db,
                    ...
                    user,
                    "card_updated",
                    f"Tarea '{old_row['title']}': {', '.join(changes)}",
                    card_id,
                )
        return _row_to_card(row)
    ...
    async def move_card(self, db, user, card_id, data):
        ...
        old_row = await db.fetchrow(
            "SELECT tc.title, tl.name as old_list FROM task_cards tc JOIN task_lists tl ON tl.id = tc.list_id WHERE tc.id = $1",
            card_id,
        )
        new_list = await db.fetchrow(
            "SELECT name FROM task_lists WHERE id = $1", data.list_id
        )
        row = await db.fetchrow(
        ...
        if board_id and old_row and new_list:
            await self._log_activity(
                ...
                user,
                "card_moved",
                f"'{old_row['title']}' de '{old_row['old_list']}' a '{new_list['name']}'",
                card_id,
            )
        return _row_to_card(row)
    ...
    async def delete_card(self, db, user, card_id):
        if not await self._card_belongs_to_user(db, card_id, user):
            raise ValueError("Card not found")
        old_row = await db.fetchrow(
            "SELECT title FROM task_cards WHERE id = $1", card_id
        )
        board_id = await self._get_board_id_for_card(db, card_id)
        await db.execute("DELETE FROM task_cards WHERE id = $1", card_id)
        if board_id and old_row:
            await self._log_activity(
                db, board_id, user, "card_deleted", f"'{old_row['title']}' eliminada"
            )
    ...
    async def delete_label(self, db, user, label_id):
        row = await db.fetchrow(
            "SELECT board_id FROM task_labels WHERE id = $1", label_id
        )
        if not row:
    ...
    async def get_board_full(self, db, user, board_id):
        if not await self._user_has_access(db, board_id, user):
            raise ValueError("Board not found")
        board_row = await db.fetchrow(
            "SELECT * FROM task_boards WHERE id = $1", board_id
        )
        ...
        for lr in list_rows:
            card_rows = await db.fetch(
                "SELECT * FROM task_cards WHERE list_id = $1 ORDER BY position",
                lr["id"],
            )
