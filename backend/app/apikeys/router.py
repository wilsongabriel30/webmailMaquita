...
async def create_api_key(body: ApiKeyCreate, request: Request, username: str = Depends(get_current_user)):
    ...
    row = await db.fetchrow(
        ...
        username, body.name, key_hash, prefix, body.permissions, expires_at,
    )
...
async def revoke_api_key(key_id: int, request: Request, username: str = Depends(get_current_user)):
    db = request.app.state.db_pool
    result = await db.execute("DELETE FROM api_keys WHERE id = $1 AND user_email = $2", key_id, username)
    if result == "DELETE 0":
