...
class OleTools(Analyzer):
    ...
    def analyze(self, path, content, report):
        try:
            r = subprocess.run(
                [OLEVBA, "--json", path], capture_output=True, text=True, timeout=30
            )
            if not (r.returncode == 0 and r.stdout.strip()):
                report.engines["oletools"] = "sin macros"
                return
            data = json.loads(r.stdout)
            report.engines["oletools"] = (
                data if isinstance(data, list) else str(data)[:500]
            )
            for item in (data if isinstance(data, list) else []):
                ...
                if item.get("type") == "AutoExec":
                    report.findings.append(
                        Finding("oletools", MALICIOUS, f"macro AutoExec: {kw}")
                    )
                elif item.get("type") == "Suspicious":
                    report.findings.append(
                        Finding("oletools", SUSPICIOUS, f"sospechoso: {kw}")
                    )
