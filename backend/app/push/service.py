...
def habilitado() -> bool:
    return bool(VAPID_PUBLIC and VAPID_PRIVATE_PATH and os.path.exists(VAPID_PRIVATE_PATH))
...
async def asegurar_tabla(db):
    ...
    """)
    await db.execute("CREATE INDEX IF NOT EXISTS ix_push_user ON push_subscriptions(username)")
...
def _enviar_sync(sub, payload):
    webpush(subscription_info=sub, data=payload,
            vapid_private_key=VAPID_PRIVATE_PATH, vapid_claims={"sub": VAPID_SUB}, timeout=10)
...
async def enviar_a_usuario(db, username, titulo, cuerpo, url="/"):
    ...
    try:
        filas = await db.fetch(
            "SELECT endpoint, p256dh, auth FROM push_subscriptions WHERE username = $1", username)
    ...
    muertas = []
    for f in filas:
        sub = {"endpoint": f["endpoint"], "keys": {"p256dh": f["p256dh"], "auth": f["auth"]}}
        try:
