...
class Evento(BaseModel):
    evento: str = Field(
        ..., max_length=80
    )  # p. ej. app_inicio, login_ok, login_fallo, sesion_sembrada, modulo_abierto, error_js, sin_conexion, actualizacion
    nivel: str = "info"
    version: str = ""
    ...
    try:
        if isinstance(v, (int, float)):
            return datetime.fromtimestamp(
                v / (1000 if v > 1e11 else 1), tz=timezone.utc
            )
...
async def recibir(request: Request):
    ...
    try:
        ...
        cuerpo = {
            "evento": "telemetria_invalida",
            "nivel": "aviso",
            "detalle": crudo.decode("utf-8", "replace"),
        }
    ...
    eventos = (
        cuerpo.get("eventos") if isinstance(cuerpo.get("eventos"), list) else [cuerpo]
    )
    ...
    async with request.app.state.db_pool.acquire() as db:
        for e in eventos[:200]:
            ...
            if nivel not in NIVELES:
                nivel = "info"
            usuario = _txt(
                e.get("usuario") or base.get("usuario") or u_cookie or "", 120
            ).lower()
            extra = e.get("extra")
            ...
            try:
                await db.execute(
                    ...
                    _fecha(e.get("ocurrido") or e.get("fecha") or e.get("ts")),
                    usuario,
                    _txt(e.get("equipo") or base.get("equipo"), 80),
                    _txt(e.get("version") or base.get("version"), 20),
                    ip[:45],
                    _txt(e.get("evento") or e.get("tipo") or "sin_nombre", 80),
                    nivel,
                    _txt(e.get("modulo"), 60),
                    _txt(e.get("url"), 300),
                    _txt(e.get("detalle") or e.get("mensaje") or e.get("message")),
                    (
                ...
                logging.getLogger(__name__).warning(
                    "telemetria: evento no guardado: %s", ex
                )
...
async def recientes(
    ...
    if nivel:
        params.append(nivel)
        cond.append(f"nivel = ${len(params)}")
    if usuario:
        params.append(f"%{usuario.lower()}%")
        cond.append(f"usuario ILIKE ${len(params)}")
    params.append(limite)
    async with request.app.state.db_pool.acquire() as db:
        rows = await db.fetch(
            f"SELECT * FROM app_telemetria WHERE {' AND '.join(cond)} ORDER BY recibido DESC LIMIT ${len(params)}",
            *params,
        )
    return [
        dict(r)
        | {
            "extra": (
                json.loads(r["extra"]) if isinstance(r["extra"], str) else r["extra"]
            )
        }
        for r in rows
    ]
...
async def resumen(
    ...
    desde = datetime.now(timezone.utc) - timedelta(hours=horas)
    async with request.app.state.db_pool.acquire() as db:
        por_version = await db.fetch(
            "SELECT version, count(DISTINCT equipo) equipos, count(DISTINCT usuario) usuarios, count(*) eventos FROM app_telemetria WHERE recibido >= $1 GROUP BY 1 ORDER BY 1 DESC",
            desde,
        )
        errores = await db.fetch(
            "SELECT usuario, equipo, version, evento, modulo, left(detalle, 200) detalle, max(recibido) ultimo, count(*) veces FROM app_telemetria WHERE recibido >= $1 AND nivel IN ('error','critico') GROUP BY 1,2,3,4,5,6 ORDER BY ultimo DESC LIMIT 100",
            desde,
        )
        logins = await db.fetch(
            "SELECT evento, count(*) FROM app_telemetria WHERE recibido >= $1 AND evento IN ('login_ok','login_fallo','sesion_sembrada','sesion_fallo') GROUP BY 1",
            desde,
        )
        equipos = await db.fetch(
            "SELECT equipo, usuario, max(version) version, max(recibido) ultimo FROM app_telemetria WHERE recibido >= $1 GROUP BY 1,2 ORDER BY ultimo DESC LIMIT 300",
            desde,
        )
    return {
        "horas": horas,
        "por_version": [dict(r) for r in por_version],
...
async def resumen_interno(request: Request, horas: int = Query(24, ge=1, le=720)):
    ...
    if not secreto or request.headers.get("X-Notif-Secret") != secreto:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Secreto invalido"
        )
    desde = datetime.now(timezone.utc) - timedelta(hours=horas)
    async with request.app.state.db_pool.acquire() as db:
        por_version = await db.fetch(
