...
def nombre(correo: str) -> str:
    ...
    return (
        _nombres["d"].get((correo or "").lower())
        or (correo or "").split("@")[0].replace(".", " ").title()
    )
...
async def emitir(
    ...
    db=None,
    card_id=None,
    tipo_registro: str = "tarea",
    incluir_originador: bool = False,
) -> int:
    """Envía el aviso `tarea` a los correos indicados (se descarta al propio originador). Devuelve destinatarios."""
    correos = sorted(
        {c for c in (correos or []) if c and (incluir_originador or c != en_nombre_de)}
    )
    if not correos:
        return 0
    cuerpo = {
    ...
    if db is not None:
        try:
            for a in correos:
                await db.execute(
                    "INSERT INTO task_avisos (card_id, tipo, a, texto, enviado) VALUES ($1,$2,$3,$4,$5)",
                    card_id,
                    tipo_registro,
                    a,
                    texto[:300],
...
async def _correo(correos, titulo, texto, url):
    ...
    try:
        ...
        msg.attach(
            MIMEText(
                f'<div style="font-family:Segoe UI,Arial,sans-serif;font-size:15px;color:#233">'
                ...
                f'<p style="color:#667;font-size:13px">{escape(PIE)}</p></div>',
                "html",
                "utf-8",
            )
        )
        await aiosmtplib.send(
            msg, hostname="127.0.0.1", port=25, start_tls=False, timeout=15
        )
    except Exception as e:
        ...
        try:
            for c in correos:
                uid = await con.fetchval(
                    """SELECT u.id FROM usuarios u LEFT JOIN trabajadores t ON t.id = u.trabajador_id
                                            WHERE lower(u.email) = $1 OR lower(t.email_institucional) = $1
                                            ORDER BY u.active DESC NULLS LAST, u.last_login DESC NULLS LAST LIMIT 1""",
                    c.lower(),
                )
                if uid:
                    await con.execute(
                        """INSERT INTO notifications (user_id, type, title, message, action_url, reference_type, is_read, created_at)
                                         VALUES ($1, 'tarea', $2, $3, $4, 'tarea', false, now())""",
                        uid,
                        titulo[:255],
                        texto,
                        url,
