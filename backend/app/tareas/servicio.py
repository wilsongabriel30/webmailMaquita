...
def a_salida(row) -> TareaOut:
    ...
    if (
        not row["completed"]
        and estado not in ("espera", "completada")
        and row["due_date"]
        and row["due_date"] < datetime.now(timezone.utc)
    ):
        estado = "vencida"
    ...
    return TareaOut(
        ...
        asignados=list(row["asignados"] or []),
        asignado_por=row["created_by"],
        plazo=row["due_date"],
        prioridad=row["priority"],
        etiquetas=_lista(row["etiquetas"]) or _lista(row["labels"]),
        estado=estado,
        semaforo=_semaforo(row),
        aceptacion=row["aceptacion"],
        motivo_rechazo=row["motivo_rechazo"],
        recurrencia=row["recurrence"],
        activa_tarea_id=row["activa_a"],
        activada_por=row["activa_tarea_id"],
        escalar_a=row["escalar_a"],
        escalado_en=row["escalado_en"],
        correo=correo,
        subtareas_total=row["st_total"],
        subtareas_hechas=row["st_hechas"],
        comentarios=row["n_com"],
        completada_por=row["completed_by"],
        completada_en=row["completed_at"],
        creada_en=row["created_at"],
        actualizada_en=row["updated_at"],
        url=avisos.url_tarea(row["id"]),
    )
...
class ServicioTareas:
    ...
    async def obtener(self, db, tarea_id, user):
        ...
        if (
            user not in (row["created_by"], *(row["asignados"] or []))
            and user != row["escalar_a"]
        ):
    ...
    async def mis(self, db, user, incluir_completadas=False):
        ...
        rows = await db.fetch(
            SELECT
            + f""" WHERE c.asignada AND $1 = ANY(COALESCE(a.asignados, '{{}}')) {cond}
                              ORDER BY c.completed, c.due_date NULLS LAST, c.created_at DESC""",
            user,
        )
    ...
    async def asignadas_por_mi(self, db, user, incluir_completadas=False):
        ...
        rows = await db.fetch(
            SELECT
            + f""" WHERE c.asignada AND c.created_by = $1 {cond}
                              ORDER BY c.completed, c.due_date NULLS LAST, c.created_at DESC""",
            user,
        )
        return [a_salida(r) for r in rows]
    ...
    async def mi_dia(self, db, user):
        ...
        rows = await db.fetch(
            SELECT + """ WHERE c.asignada AND NOT c.completed AND c.estado <> 'espera'
    ...
    async def asignar(self, db, redis, user, d: TareaAsignar):
        ...
        asignados = sorted(
            {a.strip().lower() for a in d.asignados if a and "@" in a}
        ) or [user]
        card = await task_service.create_task(
            ...
            CardCreate(
                ...
                labels=d.etiquetas,
                assigned_to=asignados[0],
                recurrence=d.recurrencia,
            ),
        )
        estado = "espera" if d.en_espera else "pendiente"
        await db.execute(
            """UPDATE task_cards SET asignada = TRUE, estado = $2, etiquetas = $3::jsonb, correo_ref = $4::jsonb,
                            activa_tarea_id = $5, escalar_a = $6 WHERE id = $1""",
            card.id,
            estado,
            json.dumps(d.etiquetas),
            json.dumps(d.correo.model_dump(by_alias=True)) if d.correo else None,
            d.activa_tarea_id,
            (d.escalar_a or "").lower() or None,
        )
        for a in asignados:
            await db.execute(
                "INSERT INTO task_asignados (card_id, email) VALUES ($1,$2) ON CONFLICT DO NOTHING",
                card.id,
                a,
            )
        for i, s in enumerate(t for t in d.subtareas if t.strip()):
            await db.execute(
                "INSERT INTO task_steps (card_id, title, position) VALUES ($1,$2,$3)",
                card.id,
                strip_html(s)[:300],
                i,
            )
        ...
        if estado != "espera":
            ...
            plazo = (
                f" · vence {row['due_date'].astimezone(EC):%d/%m %H:%M}"
                if row["due_date"]
                else ""
            )
            await avisos.emitir(
                asignados,
                f"Nueva tarea de {avisos.nombre(user)}",
                f'{row["title"]}{plazo}',
                avisos.url_tarea(card.id),
                user,
                {"tarea_id": str(card.id), "accion": "asignada"},
                db,
                card.id,
                "asignada",
            )
    ...
    async def editar(self, db, redis, user, tarea_id, d: TareaEditar):
        ...
        if d.titulo is not None:
            s("title", strip_html(d.titulo))
        if d.descripcion is not None:
            s("description", sanitize_html(d.descripcion))
        if d.quitar_plazo:
            s("due_date", None)
            s("aviso_24h_en", None)
            s("aviso_vencida_en", None)
        elif d.plazo is not None:
            s("due_date", d.plazo)
            s("aviso_24h_en", None)
            s("aviso_vencida_en", None)
            s("escalado_en", None)
        if d.prioridad in PRIORIDADES:
            s("priority", d.prioridad)
        if d.etiquetas is not None:
            s("etiquetas", json.dumps(d.etiquetas))
        if d.quitar_recurrencia:
            s("recurrence", None)
        elif d.recurrencia in RECURRENCIAS:
            s("recurrence", d.recurrencia)
        if d.activa_tarea_id is not None:
            s("activa_tarea_id", d.activa_tarea_id)
        if d.escalar_a is not None:
            s("escalar_a", d.escalar_a.lower() or None)
        if sets:
            sets.append("updated_at = NOW()")
            await db.execute(
                f'UPDATE task_cards SET {", ".join(sets)} WHERE id = $1',
                tarea_id,
                *vals,
            )
        nuevos = []
        if d.asignados is not None:
            ...
            for a in actuales:
                if a not in quiere:
                    await db.execute(
                        "DELETE FROM task_asignados WHERE card_id = $1 AND email = $2",
                        tarea_id,
                        a,
                    )
            ...
            if quiere:
                await db.execute(
                    "UPDATE task_cards SET assigned_to = $2, aceptacion = CASE WHEN $3 THEN aceptacion ELSE 'sin_responder' END WHERE id = $1",
                    tarea_id,
                    quiere[0],
                    not nuevos,
                )
        ...
        if nuevos:
            await avisos.emitir(
                ...
                f"Nueva tarea de {avisos.nombre(user)}",
                row["title"],
                avisos.url_tarea(tarea_id),
                user,
                {"tarea_id": str(tarea_id), "accion": "asignada"},
                db,
                tarea_id,
                "asignada",
            )
        ...
        if sets and viejos:
            await avisos.emitir(
                viejos,
                f"{avisos.nombre(user)} cambió la tarea",
                row["title"],
                avisos.url_tarea(tarea_id),
                user,
                {"tarea_id": str(tarea_id), "accion": "cambiada"},
                db,
                tarea_id,
                "cambiada",
            )
    ...
    async def cambiar_estado(self, db, redis, user, tarea_id, estado):
        ...
        await db.execute(
            "UPDATE task_cards SET estado = $2, completed = FALSE, completed_by = NULL, completed_at = NULL, updated_at = NOW() WHERE id = $1",
            tarea_id,
            estado,
        )
        row = await self.obtener(db, tarea_id, user)
        ...
        await avisos.emitir(
            destino,
            f"Tarea {nombres.get(estado, estado)}",
            f'{avisos.nombre(user)}: {row["title"]}',
            avisos.url_tarea(tarea_id),
            user,
            {"tarea_id": str(tarea_id), "accion": estado},
            db,
            tarea_id,
            estado,
        )
    ...
    async def aceptar(self, db, user, tarea_id):
        ...
        await db.execute(
            "UPDATE task_cards SET aceptacion = 'aceptada', motivo_rechazo = '', estado = CASE WHEN estado = 'pendiente' THEN 'en_curso' ELSE estado END, updated_at = NOW() WHERE id = $1",
            tarea_id,
        )
        row = await self.obtener(db, tarea_id, user)
        await avisos.emitir(
            [row["created_by"]],
            f"{avisos.nombre(user)} aceptó la tarea",
            row["title"],
            avisos.url_tarea(tarea_id),
            user,
            {"tarea_id": str(tarea_id), "accion": "aceptada"},
            db,
            tarea_id,
            "aceptada",
        )
    ...
    async def rechazar(self, db, user, tarea_id, motivo):
        ...
        await db.execute(
            "UPDATE task_cards SET aceptacion = 'rechazada', motivo_rechazo = $2, updated_at = NOW() WHERE id = $1",
            tarea_id,
            strip_html(motivo or "")[:500],
        )
        row = await self.obtener(db, tarea_id, user)
        await avisos.emitir(
            [row["created_by"]],
            f"{avisos.nombre(user)} rechazó la tarea",
            f'{row["title"]} — {motivo or "sin motivo"}',
            avisos.url_tarea(tarea_id),
            user,
            {"tarea_id": str(tarea_id), "accion": "rechazada"},
            db,
            tarea_id,
            "rechazada",
        )
    ...
    async def completar(self, db, redis, user, tarea_id):
        row = await self.obtener(db, tarea_id, user)
        await db.execute(
            """UPDATE task_cards SET completed = TRUE, estado = 'completada', completed_by = $2, completed_at = NOW(),
        ...
        await avisos.emitir(
            destino,
            f"Tarea completada por {avisos.nombre(user)}",
            row["title"],
            avisos.url_tarea(tarea_id),
            user,
            {"tarea_id": str(tarea_id), "accion": "completada"},
            db,
            tarea_id,
            "completada",
        )
    ...
    async def _siguiente_recurrencia(self, db, redis, row):
        ...
        if not rec or not plazo:
            return
        base = (
            max(plazo, datetime.now(timezone.utc))
            if plazo < datetime.now(timezone.utc) - timedelta(days=60)
            else plazo
        )
        if rec == "daily":
            ...
            nuevo = base + timedelta(days=1)
            while nuevo.astimezone(EC).weekday() >= 5:
                nuevo += timedelta(days=1)
        elif rec == "weekly":
            nuevo = base + timedelta(weeks=1)
        elif rec == "monthly":
            ...
            dia = min(l.day, calendar.monthrange(y, m)[1])
            nuevo = l.replace(year=y, month=m, day=dia).astimezone(timezone.utc)
        elif rec == "yearly":
            l = base.astimezone(EC)
            try:
                nuevo = l.replace(year=l.year + 1).astimezone(timezone.utc)
            except ValueError:
                nuevo = l.replace(year=l.year + 1, day=28).astimezone(timezone.utc)
        else:
            return
        ...
        d = TareaAsignar(
            ...
            plazo=nuevo,
            prioridad=row["priority"],
            etiquetas=_lista(row["etiquetas"]),
            recurrencia=rec,
            escalar_a=row["escalar_a"],
        )
        pasos = await db.fetch(
            "SELECT title FROM task_steps WHERE card_id = $1 ORDER BY position",
            row["id"],
        )
    ...
    async def _activar_cadena(self, db, redis, row, user):
        sig = await db.fetch(
            SELECT
            + " WHERE c.activa_tarea_id = $1 AND c.estado = 'espera' AND NOT c.completed",
            row["id"],
        )
        for n in sig:
            await db.execute(
                "UPDATE task_cards SET estado = 'pendiente', updated_at = NOW() WHERE id = $1",
                n["id"],
            )
            n2 = await db.fetchrow(SELECT + " WHERE c.id = $1", n["id"])
            await self._calendario(db, redis, n2, n2["created_by"])
            await avisos.emitir(
                list(n2["asignados"] or []),
                "Te toca: tarea activada",
                f'«{row["title"]}» se completó; ahora sigue «{n2["title"]}»',
                avisos.url_tarea(n2["id"]),
                user,
                {"tarea_id": str(n2["id"]), "accion": "activada"},
                db,
                n2["id"],
                "activada",
            )
    ...
    async def comentar(self, db, user, tarea_id, texto):
        ...
        c = await db.fetchrow(
            "INSERT INTO task_comentarios (card_id, autor, texto, menciones) VALUES ($1,$2,$3,$4::jsonb) RETURNING *",
            tarea_id,
            user,
            texto,
            json.dumps(menciones),
        )
        partes = set(row["asignados"] or []) | {row["created_by"]} | set(menciones)
        await avisos.emitir(
            list(partes),
            f'{avisos.nombre(user)} comentó: {row["title"]}',
            texto,
            avisos.url_tarea(tarea_id),
            user,
            {"tarea_id": str(tarea_id), "accion": "comentario"},
            db,
            tarea_id,
            "comentario",
        )
    ...
    async def comentarios(self, db, user, tarea_id):
        ...
        return await db.fetch(
            "SELECT * FROM task_comentarios WHERE card_id = $1 ORDER BY creado_en",
            tarea_id,
        )
    ...
    async def eliminar(self, db, user, tarea_id):
        ...
        await avisos.emitir(
            list(row["asignados"] or []),
            "Tarea eliminada",
            f'{avisos.nombre(user)} eliminó «{row["title"]}»',
            avisos.URL_TAREAS,
            user,
            {"accion": "eliminada"},
            db,
    ...
    async def personas(self, db, q, limite=15):
        ...
        if not hasattr(self, "_nom_cache") or ahora - self._nom_cache[0] > 60:
            try:
                ...
                logging.getLogger(__name__).warning(
                    "nómina en vivo no disponible, uso la copia: %s", e
                )
                filas = await db.fetch(
                    "SELECT email, nombre, cargo, departamento FROM task_personas ORDER BY nombre"
                )
                self._nom_cache = (
                    ahora - 50,
        ...
        def plano(x):
            return (
                unicodedata.normalize("NFKD", (x or "").lower())
                .encode("ascii", "ignore")
                .decode()
            )
        ...
        res = [
            p
            for p in self._nom_cache[1]
            if not qq
            or qq in plano(p["nombre"])
            or qq in plano(p["email"])
            or qq in plano(p["cargo"])
            or qq in plano(p["departamento"])
        ]
        return res[:limite]
    ...
    async def escalamiento_guardar(self, db, user, cfg):
        return await db.fetchrow(
            """INSERT INTO task_escalamiento (departamento, jefe_email, dias, actualizado_por)
            ...
            cfg.departamento.strip(),
            cfg.jefe_email.strip().lower(),
            max(1, cfg.dias),
            user,
        )
    ...
    async def escalamiento_borrar(self, db, departamento):
        await db.execute(
            "DELETE FROM task_escalamiento WHERE departamento = $1", departamento
        )
