...
async def main():
    ...
    async with pool.acquire() as db:
        ...
        rows = await db.fetch(
            SELECT
            + """ WHERE c.asignada AND NOT c.completed AND c.estado NOT IN ('espera','completada')
        ...
        for r in rows:
            await avisos.emitir(
                list(r["asignados"] or []),
                "Vence mañana",
                f'{r["title"]} · {r["due_date"].astimezone(EC):%d/%m %H:%M}',
                avisos.url_tarea(r["id"]),
                r["created_by"],
                {"tarea_id": str(r["id"]), "accion": "recordatorio"},
                db,
                r["id"],
                "recordatorio_24h",
            )
            await db.execute(
                "UPDATE task_cards SET aviso_24h_en = NOW() WHERE id = $1", r["id"]
            )
        n1 = len(rows)
        # 2) vencidas
        rows = await db.fetch(
            SELECT
            + """ WHERE c.asignada AND NOT c.completed AND c.estado NOT IN ('espera','completada')
                              AND c.aviso_vencida_en IS NULL AND c.due_date IS NOT NULL AND c.due_date <= $1""",
            ahora,
        )
        for r in rows:
            await db.execute(
                "UPDATE task_cards SET estado = 'vencida', aviso_vencida_en = NOW(), updated_at = NOW() WHERE id = $1",
                r["id"],
            )
            ...
            await avisos.emitir(
                asignados,
                "Tarea VENCIDA",
                f'{r["title"]} venció el {r["due_date"].astimezone(EC):%d/%m %H:%M} y sigue sin completarse',
                avisos.url_tarea(r["id"]),
                r["created_by"],
                {"tarea_id": str(r["id"]), "accion": "vencida"},
                db,
                r["id"],
                "vencida",
            )
            await avisos.emitir(
                ...
                f'{r["title"]} — asignada a {", ".join(avisos.nombre(a) for a in asignados)}',
                avisos.url_tarea(r["id"]),
                asignados[0] if asignados else "",
                {"tarea_id": str(r["id"]), "accion": "vencida"},
                db,
                r["id"],
                "vencida_asignador",
        ...
        cfg = {
            c["departamento"].lower(): c
            for c in await db.fetch("SELECT * FROM task_escalamiento")
        }
        rows = await db.fetch(
            SELECT
            + """ WHERE c.asignada AND NOT c.completed AND c.estado = 'vencida' AND c.escalado_en IS NULL
                              AND c.due_date IS NOT NULL AND c.due_date <= $1""",
        ...
        for r in rows:
            ...
            for a in r["asignados"] or []:
                dep = (
                    await db.fetchval(
                        "SELECT COALESCE(department, '') FROM user_profiles WHERE user_email = $1",
                        a,
                    )
                    or ""
                ).lower()
                if dep in cfg:
                    dias = cfg[dep]["dias"]
                if not jefe:
                    jefe = await db.fetchval(
                        "SELECT jefe_email FROM task_jefes WHERE email = $1", a
                    )
            ...
            await avisos.emitir(
                [jefe, r["created_by"]],
                f"ESCALADA: {dias} días vencida",
                f'{r["title"]} — asignada a {", ".join(avisos.nombre(a) for a in asignados)} por {avisos.nombre(r["created_by"])}',
                avisos.url_tarea(r["id"]),
                asignados[0] if asignados else r["created_by"],
                {"tarea_id": str(r["id"]), "accion": "escalada"},
                db,
                r["id"],
                "escalada",
            )
            await db.execute(
                "UPDATE task_cards SET escalado_en = NOW() WHERE id = $1", r["id"]
            )
            n3 += 1
    await pool.close()
    if n1 or n2 or n3:
