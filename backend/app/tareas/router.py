...
@router.get("/personas")
async def personas(
    request: Request, q: str = Query(""), user: str = Depends(get_current_user)
):
...
async def escalamiento_guardar(
    cfg: EscalamientoConfig, request: Request, user: str = Depends(require_admin)
):
    return dict(await svc.escalamiento_guardar(_db(request), user, cfg))


@router.delete("/escalamiento/{departamento}", status_code=204)
async def escalamiento_borrar(
    departamento: str, request: Request, user: str = Depends(require_admin)
):
...
async def asignar(
    d: TareaAsignar, request: Request, user: str = Depends(get_current_user)
):
    try:
...
async def obtener(
    tarea_id: uuid.UUID, request: Request, user: str = Depends(get_current_user)
):
    try:
...
async def estado(
    ...
    try:
        return await svc.cambiar_estado(
            _db(request), _redis(request), user, tarea_id, d.estado
        )
    except Exception as e:
        raise _http(e)
...
def _com(r):
    ...
    return ComentarioOut(
        ...
        autor=r["autor"],
        texto=r["texto"],
        menciones=json.loads(m) if isinstance(m, str) else list(m or []),
