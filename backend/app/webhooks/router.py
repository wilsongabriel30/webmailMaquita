...
async def create_webhook(
    body: WebhookCreate, request: Request, username: str = Depends(get_current_user)
):
...
async def list_webhooks(request: Request, username: str = Depends(get_current_user)):
    db = request.app.state.db_pool
    rows = await db.fetch(
        "SELECT * FROM webhooks WHERE user_email = $1 ORDER BY id", username
    )
...
async def get_webhook(
    webhook_id: int, request: Request, username: str = Depends(get_current_user)
):
    db = request.app.state.db_pool
    row = await db.fetchrow(
        "SELECT * FROM webhooks WHERE id = $1 AND user_email = $2", webhook_id, username
    )
    if not row:
    ...
    if body.is_active is not None:
        ...
        if body.is_active:
            ...
            try:
                ...
                raise HTTPException(
                    400,
                    detail=f"URL del webhook no valida: {e}. Actualice la URL primero.",
                )
...
async def delete_webhook(
    ...
    result = await db.execute(
        "DELETE FROM webhooks WHERE id = $1 AND user_email = $2", webhook_id, username
    )
    if result == "DELETE 0":
        raise HTTPException(404, "Webhook not found")
    return None
...
async def get_webhook_logs(
    ...
    db = request.app.state.db_pool
    row = await db.fetchrow(
        "SELECT id FROM webhooks WHERE id = $1 AND user_email = $2",
        webhook_id,
        username,
    )
    if not row:
...
async def _fire_webhook(db, webhook: dict, event: str, payload: dict):
    ...
    if resp_status and 200 <= resp_status < 300:
        ...
        if new_count >= 5:
            ...
            logger.warning(
                "Webhook %s disabled after %d failures", webhook["id"], new_count
            )
        else:
            await db.execute(
