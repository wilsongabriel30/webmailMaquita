...
def _heuristic_score(
    ...
    urgency_words = len(
        re.findall(
            r"\b(urgente|urgent|ahora|now|inmediato|immediate|último aviso|last chance|final warning)\b",
            text,
            re.IGNORECASE,
        )
    )
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=IA_TIMEOUT) as client:
                resp = await client.post(
                    IA_URL, json={"emails": batch}, headers=IA_HEADERS
                )
    ...
    if auto_move and spam_uids:
        try:
            ...
            if ok:
                moved = len(spam_uids)
                logger.info(
                    f"SpamGuard: moved {moved} spam messages to Junk for {user}"
                )
        except Exception as e:
