...
async def _es_alias_o_identidad(db, username: str, email: str) -> bool:
    ...
    if fila and username.lower() in (fila["goto"] or "").lower():
        return True
    fila = await db.fetchrow(
        "SELECT 1 FROM user_identities WHERE username = $1 AND lower(email) = $2",
        username,
        email,
    )
    return bool(fila)
...
async def resolver_remitente(
    db, username: str, from_email: str | None, display_name: str
):
    ...
    raise HTTPException(
        status_code=403, detail=f"No tienes permiso para enviar como {email}"
    )
