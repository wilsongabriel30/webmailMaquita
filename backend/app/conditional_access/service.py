    ...
    try:
        if strongest == "bloquear":
            ...
            await db.execute(
                ...
                username,
                f"Bloqueado por acceso condicional ({country}, riesgo {risk})",
            )
            ...
            await db.execute(
                ...
                username,
                f"Requiere 2FA por acceso condicional ({country})",
            )
        else:
            await db.execute(
                "INSERT INTO threat_actions (action,target,detail,actor,auto) "
                ...
                username,
                f"Alerta de acceso condicional ({country}, riesgo {risk})",
            )
