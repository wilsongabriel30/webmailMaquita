...
async def existing_uids(db, username, folder="INBOX"):
    rows = await db.fetch(
        "SELECT msg_uid FROM rag_chunks WHERE username=$1 AND folder=$2",
        username,
        folder,
    )
...
async def search(db, username, query_emb, k=6):
    return await db.fetch(
        ...
        "WHERE username=$2 ORDER BY embedding <=> $1::vector LIMIT $3",
        str(query_emb),
        username,
        k,
    )
...
async def count(db, username):
    return (
        await db.fetchval("SELECT count(*) FROM rag_chunks WHERE username=$1", username)
        or 0
    )
