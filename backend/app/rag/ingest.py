...
async def ingest_user(db, username, limit=None):
    ...
    for m in msgs:
        ...
        if not emb:
            continue
        await store.upsert(db, username, "INBOX", uid, m.get("subject", ""), m.get("from", ""), content, emb)
        indexed += 1
    return {"username": username, "indexed": indexed, "total": await store.count(db, username)}
