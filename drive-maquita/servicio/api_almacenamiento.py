...
from almacen_bd import es_master, consultar, ejecutar
...
@bp_almacenamiento.route('/preferencias', methods=['GET', 'POST'])
def preferencias():
    """GET/POST /preferencias — vista (lista/cuadrícula), orden, etc.

    PERSISTIDAS por usuario en `preferencias_usuario` (12/08/2026): antes era
    un stub que descartaba el POST, así que la vista Lista volvía a cuadrícula
    en cada recarga de página y al cerrar el navegador. El POST manda solo lo
    que cambió; se fusiona por clave (JSONB ||) sobre lo ya guardado."""
    ...
    if request.method == 'POST':
        ...
        try:
            ejecutar("""
                INSERT INTO preferencias_usuario (usuario_id, datos, actualizado_en)
                VALUES (%s, %s::jsonb, NOW())
                ON CONFLICT (usuario_id)
                DO UPDATE SET datos = preferencias_usuario.datos || EXCLUDED.datos,
                              actualizado_en = NOW()
            """, (usuario, _json.dumps(cambios)))
        except Exception as excepcion:
            log.warning('No se pudieron guardar preferencias de %s: %s',
                        usuario, excepcion)
            return error('No se pudo guardar', 500)
        return jsonify({'success': True})
    try:
        filas = consultar('SELECT datos FROM preferencias_usuario '
                          'WHERE usuario_id = %s', (usuario,))
        prefs = filas[0]['datos'] if filas else {}
    except Exception as excepcion:
        log.warning('No se pudieron leer preferencias de %s: %s', usuario, excepcion)
        prefs = {}
