    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-white rounded border border-ms-gray-30 w-[28rem] max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="px-5 py-4 border-b border-ms-gray-30 flex items-center justify-between">
          ...
          <button onClick={onClose} className="text-ms-gray-60 hover:text-ms-gray-130 text-lg leading-none">&times;</button>
        </div>

        <div className="flex border-b border-ms-gray-30">
          <button onClick={() => setTab("password")} className={`px-4 py-2 text-sm ${tab === "password" ? "border-b-2 border-ms-blue text-ms-blue font-medium" : "text-ms-gray-90"}`}>Contraseña</button>
          <button onClick={() => setTab("totp")} className={`px-4 py-2 text-sm ${tab === "totp" ? "border-b-2 border-ms-blue text-ms-blue font-medium" : "text-ms-gray-90"}`}>Verificación en dos pasos</button>
        </div>

        {tab === "password" && (
          <div className="p-5 space-y-3">
            <input type="password" placeholder="Contraseña actual" autoComplete="current-password" value={pwForm.current} onChange={(e) => setPwForm({ ...pwForm, current: e.target.value })} className="w-full px-3 py-2 border border-ms-gray-40 rounded text-sm focus:outline-none focus:border-ms-blue" />
            <input type="password" placeholder="Nueva contraseña (min 8)" autoComplete="new-password" value={pwForm.next} onChange={(e) => setPwForm({ ...pwForm, next: e.target.value })} className="w-full px-3 py-2 border border-ms-gray-40 rounded text-sm focus:outline-none focus:border-ms-blue" />
            <input type="password" placeholder="Confirmar nueva contraseña" autoComplete="new-password" value={pwForm.confirm} onChange={(e) => setPwForm({ ...pwForm, confirm: e.target.value })} onKeyDown={(e) => { if (e.key === "Enter") changePassword(); }} className="w-full px-3 py-2 border border-ms-gray-40 rounded text-sm focus:outline-none focus:border-ms-blue" />
          ...
          <div className="p-5 space-y-3">
            ...
            {totpEnabled === false && !setup && (
              <>
                ...
                <input type="password" placeholder="Confirme su contraseña para comenzar" value={totpPw} onChange={(e) => setTotpPw(e.target.value)} className="w-full px-3 py-2 border border-ms-gray-40 rounded text-sm focus:outline-none focus:border-ms-blue" />
                <div className="flex justify-end">
            ...
            {setup && (
              <>
                ...
                <input type="text" inputMode="numeric" maxLength={6} placeholder="000000" value={totpCode} onChange={(e) => setTotpCode(e.target.value.replace(/\D/g, ""))} className="w-full px-3 py-2 border border-ms-gray-40 rounded text-sm tracking-widest text-center focus:outline-none focus:border-ms-blue" />
                <div className="flex justify-end gap-2">
                  <button onClick={() => { setSetup(null); setTotpCode(""); }} className="px-4 py-2 border border-ms-gray-40 rounded text-sm text-ms-gray-90">Cancelar</button>
                  <button onClick={verifyTotp} disabled={totpCode.length !== 6} className="px-4 py-2 bg-ms-blue text-white rounded text-sm hover:bg-ms-blue-dark disabled:opacity-50">Verificar y activar</button>
                </div>
              </>
            )}
            ...
            {totpEnabled === true && !setup && (
              <>
                ...
                <input type="password" placeholder="Confirme su contraseña para desactivar" value={totpPw} onChange={(e) => setTotpPw(e.target.value)} className="w-full px-3 py-2 border border-ms-gray-40 rounded text-sm focus:outline-none focus:border-ms-blue" />
                <div className="flex justify-end">
