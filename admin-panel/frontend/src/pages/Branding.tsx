        ...
        <div className="flex-1">
          ...
          <div className="flex gap-2">
            <input ref={ref} type="file" accept={accept} className="hidden"
              title={type === "favicon" ? "Seleccione el archivo del favicon: ICO, PNG o SVG de 32x32 o 64x64 px." : "Seleccione el archivo del logo: PNG, JPG, SVG o WebP de máximo 2MB."}
              onChange={(e) => { if (e.target.files?.[0]) onUpload(e.target.files[0]); }} />
            <button onClick={() => ref.current?.click()}
              title={`${currentUrl ? "Reemplaza" : "Sube"} ${type === "favicon" ? "el favicon (icono de la pestaña del navegador)" : "el logo institucional"}. Se aplica inmediatamente en el login y el webmail de todos los usuarios.`}
            ...
            {currentUrl && (
              <button onClick={onDelete}
                title={`PRECAUCIÓN: Elimina ${type === "favicon" ? "el favicon" : "el logo"} actual del servidor. El login y el webmail volverán a mostrar la imagen por defecto inmediatamente.`}
    ...
    <div className="p-6 max-w-3xl">
      ...
      <div className="mb-6">
        ...
        <SectionHelp titulo="Personalización" items={[
          ...
          { titulo: "Guardar cambios", desc: "Los textos y el color solo se aplican al pulsar Guardar cambios. El punto ámbar indica que hay cambios pendientes sin guardar." },
        ]} />
        </div>
      ...
      {error && (
        <div className="mb-4 p-3 bg-red-50 border border-ms-red/30 rounded text-ms-red text-sm flex items-center gap-2">
          ...
          {error}
          <button onClick={() => setError("")} title="Cierra este mensaje de error. No deshace ninguna acción." className="ml-auto text-ms-red/60 hover:text-ms-red">&times;</button>
        </div>
      )}
      {saved && (
      ...
      <div className="bg-white rounded border border-ms-gray-30 p-5 mb-6">
        ...
        <div className="flex items-center gap-4">
          <input type="color" value={colorInput} onChange={(e) => setColorInput(e.target.value)}
            title="Abre el selector de color para elegir el color primario de la interfaz. Se aplica al pulsar Guardar cambios."
          ...
          <input type="text" value={colorInput} onChange={(e) => setColorInput(e.target.value)}
            title="Código hexadecimal del color primario, en formato #RRGGBB (ejemplo: #0078d4). Máximo 7 caracteres. Se aplica al pulsar Guardar cambios."
          ...
          {FIELDS.map((f) => (
            <div key={f.key}>
              ...
              <input
                ...
                title={`${f.label} de su organización (${f.placeholder}). Se mostrará en el login, el webmail y los correos del sistema. Se aplica al pulsar Guardar cambios.`}
      ...
      <div className="flex items-center gap-3">
        <button onClick={handleSave} disabled={saving}
          title="Guarda los textos de la organización y el color primario en el servidor. Los cambios se aplican inmediatamente en el login, el webmail y los correos del sistema para todos los usuarios."
