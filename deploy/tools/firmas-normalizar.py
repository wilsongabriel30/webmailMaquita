...
TABLAS = (
    # (tabla, columna con la firma, columna del dueño)
    ("user_signatures", "html_content", "owner"),
    ("user_identities", "signature_html", "username"),
    ("user_preferences", "signature_html", "username"),
    ("mail_signatures", "html_content", "domain"),
    ("mail_user_signatures", "custom_html", "username"),
)


async def main(aplicar: bool, forzar: bool) -> int:
    con = await asyncpg.connect(get_settings().database_url)
    totales = {"revisadas": 0, "sin_cambios": 0, "normalizadas": 0, "saltadas": 0, "imagenes": 0}
    try:
        for tabla, columna, duenio in TABLAS:
            try:
                filas = await con.fetch(
                    f"SELECT id, {duenio} AS duenio, {columna} AS html FROM {tabla} "
                    f"WHERE coalesce({columna}, '') <> '' ORDER BY id"
                )
            ...
            for fila in filas:
                ...
                for aviso in r.avisos:
                    print(f"  {tabla} id={fila['id']} ({fila['duenio']}): {aviso}")
                if r.html == fila["html"]:
                    n["sin_cambios"] += 1
                    continue
                if any(a.startswith("Se quitó la imagen") for a in r.avisos) and not forzar:
                    n["saltadas"] += 1
                    print(f"  {tabla} id={fila['id']}: se deja como está (imagen no recuperable; --forzar la aplicaría)")
                    continue
                n["normalizadas"] += 1
                if aplicar:
                    await con.execute(f"UPDATE {tabla} SET {columna} = $1 WHERE id = $2", r.html, fila["id"])
            for k in n:
                totales[k] += n[k]
            print(f"{tabla}: {len(filas)} con firma, {n['normalizadas']} normalizadas, "
                  f"{n['sin_cambios']} ya correctas, {n['saltadas']} saltadas")
    finally:
        await con.close()
    print(f"total: {totales['revisadas']} revisadas, {totales['normalizadas']} normalizadas, "
